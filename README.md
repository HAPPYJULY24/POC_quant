...
* **多态因子框架**：以抽象基类 `BaseFactor` 为契约，核心的 `ZScoreArbitrageFactor` 派生类对价差（Spread）进行滑动标准差 and 均值计算，支持任何新因子的动态插拔与即跨即用。
...
├── bursa_poc.db                # SQLite 历史 aligned 数据库 & 仓位状态快照 (运行后自动创建，已被 .gitignore 忽略)
└── poc_quant.log               # 系统运行全局回执日志 (运行后自动创建，已被 .gitignore 忽略)
```
...
### 2. 运行自动化测试套件
在主程序上线前，必须确保本地 26 个单元测试全部通过，在项目根目录下执行：
